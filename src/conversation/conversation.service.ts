...
export class ConversationService {
  ...
  async findBetweenUsers(users1 : string, user2 : string) : Promise<Conversation | null>{

    const id1 = new Types.ObjectId(users1)
    const id2 = new Types.ObjectId(user2)

    return this.conversationModel.findOne({
        participants : {
            $all : [id1, id2], isActive : true
        }
    }).exec()
  }
  ...
  async getMessages(conversationId : string, limit = 50, skip = 0) : Promise<Message[]>{
    const conversation = await this.conversationModel.findById(conversationId).select('messages').exec();
